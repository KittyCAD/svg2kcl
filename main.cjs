...
class PathState {
  ...
  handle_command() {
    ...
    switch (this.command) {
      case Command.NotSet: {
        break /* Do nothing when the command is not set. */
      }
      ...
      case Command.QuadraticBezierRelative: {
        for (let args of this.values.chunks(4)) {
          ...
          console.log(`|> bezierCurve({
              control1: [
                ${this.current_point.x + args[0] + this.offsetCoords.x}, ${
            -(this.current_point.y + args[1]) + this.offsetCoords.y
          }
                ],
              control2: [
                ${this.current_point.x + args[0] + this.offsetCoords.x}, ${
            -(this.current_point.y + args[1]) + this.offsetCoords.y
              ...
              to: [
                ${this.current_point.x + args[2] + this.offsetCoords.x}, ${
            -(this.current_point.y + args[3]) + this.offsetCoords.y
          }
              ]
         }, %)`)
      ...
      case Command.QuadraticBezierSmoothAbsolute: {
        break
      }
      case Command.QuadraticBezierSmoothRelative: {
        break
      }
      case Command.CubicBezierAbsolute: {
        for (let args of this.values.chunks(6)) {
          console.log(`|> bezierCurve({
    control1: [ ${args[0] - this.current_point.x + this.offsetCoords.x}, ${
            -args[1] + this.current_point.y + this.offsetCoords.y
          } ],
    control2: [ ${args[2] - this.current_point.x + this.offsetCoords.x}, ${
            -args[3] + this.current_point.y + this.offsetCoords.y
          } ],
    to: [ ${args[4] - this.current_point.x + this.offsetCoords.x}, ${
            -args[5] + this.current_point.y + this.offsetCoords.y
          } ]
}, %)`)
  ...
  to: [ ${args[4] + this.offsetCoords.x}, ${-args[5] + this.offsetCoords.y} ]
      ...
      case Command.CubicBezierSmoothAbsolute: {
        break
      }
      case Command.CubicBezierSmoothRelative: {
        break
      }
      case Command.EllipticalArcAbsolute: {
        break
      }
      case Command.EllipticalArcRelative: {
        break
      }
            ...
            switch (attr.name) {
              case 'd': {
                ...
                for (let char of attr.value.split('')) {
                  switch (char) {
                    case 'M':
                      state.push_command(Command.MoveAbsolute)
                      break
                    case 'm':
                      state.push_command(Command.MoveRelative)
                      break
                    case 'L':
                      state.push_command(Command.LineAbsolute)
                      break
                    case 'l':
                      state.push_command(Command.LineRelative)
                      break
                    case 'H':
                      state.push_command(Command.HorizontalLineAbsolute)
                      break
                    case 'h':
                      state.push_command(Command.HorizontalLineRelative)
                      break
                    case 'V':
                      state.push_command(Command.VerticalLineAbsolute)
                      break
                    case 'v':
                      state.push_command(Command.VerticalLineRelative)
                      break
                    case 'C':
                      state.push_command(Command.CubicBezierAbsolute)
                      break
                    case 'c':
                      state.push_command(Command.CubicBezierRelative)
                      break
                    case 'S':
                      state.push_command(Command.CubicBezierSmoothAbsolute)
                      break
                    case 's':
                      state.push_command(Command.CubicBezierSmoothRelative)
                      break
                    case 'Q':
                      state.push_command(Command.QuadraticBezierAbsolute)
                      break
                    case 'q':
                      state.push_command(Command.QuadraticBezierRelative)
                      break
                    case 'T':
                      state.push_command(Command.QuadraticBezierSmoothAbsolute)
                      break
                    case 't':
                      state.push_command(Command.QuadraticBezierSmoothRelative)
                      break
                    case 'A':
                      state.push_command(Command.EllipticalArcAbsolute)
                      break
                    case 'a':
                      state.push_command(Command.EllipticalArcRelative)
                      break
                    case 'Z':
                      state.push_command(Command.StopAbsolute)
                      break
                    case 'z':
                      state.push_command(Command.StopRelative)
                      break
                    case ',': {
                      state.push_value()
                      break
